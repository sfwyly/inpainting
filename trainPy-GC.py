...
def getNetwork():
    ...
    def Conv(xin,cnum = 32,ksize = 3,stride = 1, rate = 1,name = "conv",padding="same",activation=tf.nn.elu,training = True):
        
        x = layers.Conv2D(cnum,ksize,stride,dilation_rate=  rate,padding = padding,activation=activation,name = name)(xin)
        x_mask = layers.Conv2D(cnum,ksize,stride,dilation_rate=  rate,padding = padding,activation="sigmoid",name = name+"_mask")(xin)
        return x*x_mask
    #阶段2
    cnum = 32
    inputs = layers.Input(batch_shape = (6,256,256,3))
    mask = layers.Input(batch_shape = (6,256,256,3))
    xin = tf.concat([inputs,mask],axis = -1)
    
    x  =  Conv(xin,cnum,5,1,name = "conv1")
    x = Conv(x,2*cnum,3,2,name = "conv2_downsample")
    x = Conv(x,2*cnum,3,1,name = "conv3")
    x = Conv(x,4*cnum,3,2,name = "conv4_downsample")
    x = Conv(x,4*cnum,3,1,name = "conv5")
    x = Conv(x,4*cnum,3,1,name = "conv6")
    
    x = Conv(x,4*cnum,3,rate = 2,name = "conv7_atrous")
    x = Conv(x,4*cnum,3,rate = 4,name = "conv8_atrous")
    x = Conv(x,4*cnum,3,rate = 8,name = "conv9_atrous")
    x = Conv(x,4*cnum,3,rate = 16,name = "conv10_atrous")
    x = Conv(x,4*cnum,3,1,name = "conv11")
    x = Conv(x,4*cnum,3,1,name = "conv12")
    
    x = layers.UpSampling2D(size=(2,2),name = "up13")(x)
    x = Conv(x,2*cnum,3,1,name = "conv13_upsample")
    x = Conv(x,2*cnum,3,1,name = "conv14")
    
    x = layers.UpSampling2D(size=(2,2),name = "up15")(x)
    x = Conv(x,cnum,3,1,name = "conv15_upsample")
    x = Conv(x,cnum//2,3,1,name = "conv16")
    
    x = Conv(x,3,3,1,activation=None,name="conv17")
    ...
    xnow = x
    #阶段2
    x  =  Conv(xnow,cnum,5,1,name = "xconv1")
    x = Conv(x,cnum,3,2,name = "xconv2_downsample")
    x = Conv(x,2*cnum,3,1,name = "xconv3")
    x = Conv(x,2*cnum,3,2,name = "xconv4_downsample")
    x = Conv(x,4*cnum,3,1,name = "xconv5")
    x = Conv(x,4*cnum,3,1,name = "xconv6")
    #mask_s = resize_mask_like(mask, x)
    mask_s = tf.image.resize(mask,(x.shape)[1:3],"bilinear")
    x = Conv(x,4*cnum,3,rate = 2,name = "xconv7_atrous")
    x = Conv(x,4*cnum,3,rate = 4,name = "xconv8_atrous")
    x = Conv(x,4*cnum,3,rate = 8,name = "xconv9_atrous")
    x = Conv(x,4*cnum,3,rate = 16,name = "xconv10_atrous")
    
    x_hallu = x
    
    #attention branch
    x = Conv(xnow,cnum,5,1,name = "pmconv1")
    x = Conv(x,cnum,3,2,name = "pmconv2_downsample")
    x = Conv(x,2*cnum,3,1,name="pmconv3")
    x = Conv(x,4*cnum,3,2,name="pmconv4_downsample")
    x = Conv(x,4*cnum,3,1,name="pmconv5")
    x = Conv(x,4*cnum,3,1,name="pmconv6")
    #注意力
    x = ContextualAttention()(x,mask_s)
    x = Conv(x,4*cnum,3,1,name = "pmconv9")
    x = Conv(x,4*cnum,3,1,name = "pmconv10")
    pm = x
    x = tf.concat([x_hallu,pm],axis = -1)
    
    x = Conv(x,4*cnum,3,1,name = "allconv11")
    x = Conv(x,4*cnum,3,1,name = "allconv12")
    x = layers.UpSampling2D(size=(2,2),name = "allup13")(x)
    x = Conv(x,2*cnum,3,1,name = "allconv13_upsample")
    x = Conv(x,2*cnum,3,1,name = "allconv14")
    x = layers.UpSampling2D(size=(2,2),name = "allup15")(x)
    x = Conv(x,cnum,3,1,name = "allconv15_upsample")
    x = Conv(x,cnum//2,3,1,name = "allconv16")
    x = Conv(x,3,3,1,activation="sigmoid",name="allconv17")
...
#generator.load_weights("/home/oyx/jjj/inpainting/deepfashion/GC_deepfashion.h5")
...
#         print(X_.shape,mask_list.shape)
      ...
      if((load_train+1)%1 ==0):
        generator.save_weights("/home/oyx/jjj/inpainting/deepfashion/GC_deepfashion.h5")
        #generator.save_weights("/root/sfwy/inpainting/generator_partial_attention_Place2.h5")
  return loss_list


if(__name__=="__main__"):
  all_image_paths = getAllImagePath("/home/oyx/jjj/inpainting/deepfashion/deepfashion_child/")#/root/sfwy/inpainting/CeleAHQ/img_align_celeba/ /root/sfwy/inpainting/test_256/
  #all_image_paths = getAllImagePath("/root/sfwy/inpainting/place2/data_256/z/zen_garden/") #/root/sfwy/inpainting/Paris_StreetView_Dataset/paris_train_original/
#   np.random.shuffle(all_image_paths)
