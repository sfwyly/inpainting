...
def getNetwork():
    ...
    mask = layers.Input(shape = (256,256,3))
    ...
    x = Conv(3,3,1,activation=None,name="conv17")(x)
    ...
    x = Conv(4*cnum,3,1,name = "xconv4_2")(x)
    
    x_t = x
    # 残差推理
    for _ in range(6):
        
        x  = tf.concat([x_t,x],axis = -1)
        x = Conv(4*cnum,3,1,name = "xconv5_"+str(_))(x)
        x = Conv(4*cnum,3,1,name = "xconv6_"+str(_))(x)
        #mask_s = resize_mask_like(mask, x)
        x = Conv(4*cnum,3,rate = 2,name = "xconv7_atrous_"+str(_))(x)
        x = Conv(4*cnum,3,rate = 4,name = "xconv8_atrous_"+str(_))(x)
        x = Conv(4*cnum,3,rate = 8,name = "xconv9_atrous_"+str(_))(x)
        x = Conv(4*cnum,3,rate = 16,name = "xconv10_atrous_"+str(_))(x)
...
#     x = tf.concat([x_hallu,pm],axis = -1)
    ...
    x = Conv(3,3,1,activation=None,name="allconv17")(x)
...
#generator.load_weights("/root/sfwy/inpainting/RR_CeleA.h5")
...
def cal_gen(fake_image_list):
  return cross_entropy(tf.ones_like(fake_image_list), clip_0_1s(fake_image_list))
...
#   hole_list = getHoles((256,256),holesNum)[...,np.newaxis]
  ...
  for epoch in range(epochs):
    ...
    for load_train in range(int(np.ceil(len(all_image_paths)/(20*batch_size)))): #最后一个迭代用来测试 train_length//(10*batch_size) -1
      #初始化数据
      X_train = getDataset(all_image_paths[load_train*(20*batch_size):(load_train+1)*(20*batch_size)])/127.5  - 1
...
#         mask_list = hole_list[np.random.randint(holesNum)][np.newaxis,...]
      ...
      if((load_train+1)%10 ==0):
        generator.save_weights("/root/sfwy/inpainting/RR_CeleA.h5")
